    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
                .flatMapToPair(kv -> {
                    ...
                    Website source = value._1();
                    double currentRank = value._2();
                    int outCount = source.getNEdges();
                    Iterator<Integer> iterator = source.edgeIterator();
                    while (iterator.hasNext()) {
                        int target = iterator.next();
                        contributions.add(new Tuple2<>(target, currentRank / (double) outCount));
                    }
                    return contributions;
                });
        return newRanks.reduceByKey((contribution1, contribution2) -> contribution1 + contribution2)
