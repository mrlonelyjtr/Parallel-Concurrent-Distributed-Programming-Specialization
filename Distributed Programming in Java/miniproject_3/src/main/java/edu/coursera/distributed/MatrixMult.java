    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        if (rank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                ...
                rankEndRow = rankEndRow > rows ? rows : rankEndRow;
            ...
            final int rowOffset = startRow * c.getNCols();
            final int nElements = (endRow - startRow) * c.getNCols();
            mpi.MPI_Send(c.getValues(), rowOffset, nElements, 0, rank, mpi.MPI_COMM_WORLD);
        }
    }
}
